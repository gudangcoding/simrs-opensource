...
export default function Edit({ payment, patients }) {
    ...
    return (
        <AuthenticatedLayout header={`Edit Payment #${payment.id}`}>
            <FlashMessage />
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white shadow rounded-lg p-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <Field label="Payment Date" htmlFor="payment_date" required>
                            <Input id="payment_date" type="datetime-local" value={data.payment_date?.slice(0, 16) || ''} onChange={(e) => setData('payment_date', e.target.value)} required error={errors.payment_date} />
                        </Field>
